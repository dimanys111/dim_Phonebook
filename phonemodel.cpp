...
void PhoneModel::set_sort(bool b)
{
    if (b) {
...
bool PhoneModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == NameRole)
        person.m_name = value.toString();
    else if (role == PhoneRole)
        person.m_phone = value.toString();
    else if (role == AddressRole)
        person.m_address = value.toString();
...
bool PhoneModel::save(const QUrl& file_name)
{
    QString s = file_name.path();
    if (s.length() < 0) {
        return false;
    }
#ifdef _WIN32
...
bool PhoneModel::open(const QUrl& file_name)
{
    QString s = file_name.path();
    if (s.length() < 0) {
        return false;
    ...
    if (file.open(QFile::ReadOnly)) {
        ...
        QDataStream ds(&file);
        while (!ds.atEnd()) {
            addPerson(Person::from_data_stream(ds));
        }
        update();
        file.close();
        return true;
